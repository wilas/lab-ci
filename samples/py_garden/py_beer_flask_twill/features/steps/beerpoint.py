...
def step(context):
    ...
    assert context.browser.get_html().decode('utf-8')

@then('We see beer point name')
def step(context):
    assert re.search('beer point name: \w+', context.browser.get_html().decode('utf-8'))
...
def step(context,type):
    #print repr(type)
    #print repr(context.browser.get_html())
    assert type in context.browser.get_html().decode('utf-8')
...
def step(context,text):
    ...
    beerslist = BeautifulSoup(context.browser.get_html().decode('utf-8')).findAll('li')
    context.beerslist = []
    ...
    assert re.search(pattern,context.browser.get_html().decode('utf-8'))
